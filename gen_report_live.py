...
Credentials:
    Create a file called  credentials_zoho.py  in the same folder with:
...
# ── SharePoint credentials — loaded from credentials_zoho.py (never committed) ─
...
def fetch_by_status(status: str, token: str) -> list[dict]:
    tickets, from_ = [], 0
    while True:
        resp = requests.get(f"{ZOHO_API_BASE}/tickets", headers=_hdrs(token), params={
...
def fetch_all_recent_alerts(token: str, days_back: int = 8) -> list[dict]:
    """Fetch SRE alert tickets from the last `days_back` days across all statuses."""
    ...
    def _mini_table(env: str, color: str) -> str:
        ...
        for j, atype in enumerate(all_types):
            ...
            if td == 0 and yd == 0 and wd == 0:
                continue
            if td == 0 and yd == 0:
                today_cell = '<span style="font-size:12px;color:#CBD5E1;">&ndash;</span>'
            elif td > yd:
                today_cell = (
                ...
                today_cell = (
                    '<span style="font-size:13px;font-weight:800;color:#16A34A;">0</span>'
                    '&thinsp;<span style="font-size:9px;color:#16A34A;vertical-align:middle;">&#9660;</span>'
                )
            else:
                today_cell = (
                    f'<span style="font-size:13px;font-weight:800;color:#1E293B;">{td}</span>'
                    '&thinsp;<span style="font-size:9px;color:#94A3B8;vertical-align:middle;">=</span>'
                )
...
def main():
    ...
    def enrich(t: dict) -> dict:
        ...
        try:
            ...
            if detail.get("contacts"):  t["contacts"] = detail["contacts"]
            if detail.get("contact"):   t["contact"]  = detail["contact"]
            if detail.get("assignee"):  t["assignee"] = detail["assignee"]
            if detail.get("account"):   t["account"]  = detail["account"]
            ...
            if detail.get("type"):       t["type"]       = detail["type"]
    ...
    # Generate Excel and append Alerts sheet
    ...
    # Upload Excel first to get SharePoint URL for the HTML footer
    ...
    # Generate HTML with alert widget injected
    html = generate_html(tickets, today, excel_url=excel_sp_url)
    ...
    logging.info("Done. %d tickets in report. Today alerts: %d", len(tickets), len(today_alerts))

"""
perf_report.py
CoreStack Platform Performance Report Generator — Azure Function edition.
Adapted from Platform_Performance_Report.py to return bytes instead of writing files.
"""
import csv
...
RAW_DATA_URL = (
    "https://cloudenablersinc.sharepoint.com/sites/SupportTeam/Shared%20Documents/Forms/AllItems.aspx"
    "?id=%2Fsites%2FSupportTeam%2FShared%20Documents%2FGeneral%2FCost%2DPerformance%2DReport%2FDump"
    "&viewid=b4659049%2Ddfdc%2D4431%2Dabd5%2D3e6db977a8dd&sharingv2=true&fromShare=true&at=9"
    "&CT=1779343271062&OR=OWA%2DNT%2DMail&CID=d8d72bc7%2Dd267%2D4b2d%2D588a%2D2136ada0b6c5"
    "&clickParams=eyJYLUFwcE5hbWUiOiJNaWNyb3NvZnQgT3V0bG9vayBXZWIgQXBwIiwiWC1BcHBWZXJzaW9uIjoiMjAyNjA1MDgwMTAuMTIiLCJPUyI6IldpbmRvd3MgMTEifQ%3D%3D"
    "&FolderCTID=0x012000BB629520B6C8374787B595617D6FDD7E"
)
...
def get_job_metrics(env_key: str, now: datetime) -> dict:
    ...
    try:
        ...
        logging.warning("jobs/%s: %s", env_key, exc)
    finally:
        if client:
            client.close()
    return result
...
def get_audit_metrics(env_key: str, now: datetime) -> dict:
    ...
    try:
        ...
        result["error"] = str(exc)
        logging.warning("audit/%s: %s", env_key, exc)
    finally:
        if client:
            client.close()
    return result
...
def get_audit_raw_data(env_key: str, now: datetime) -> list:
    ...
    try:
        ...
        logging.warning("raw_data/%s: %s", env_key, exc)
    finally:
        if client:
            client.close()
    return rows
...
def generate_csv_bytes(now: datetime) -> bytes:
    all_rows = []
    for env_key in ENVIRONMENTS:
        logging.info("[%s] fetching raw data...", env_key)
        rows = get_audit_raw_data(env_key, now)
        all_rows.extend(rows)
        logging.info("[%s] %d raw records", env_key, len(rows))
...
# ============================================================
#  PUBLIC API — called by function_app.py
# ============================================================

def generate_report(now: datetime) -> tuple[bytes, bytes, str, str]:
    """
    Run the full report pipeline.
    Returns: (html_bytes, csv_bytes, html_filename, csv_filename)
    """
    ist = now + timedelta(hours=5, minutes=30)
    logging.info("Performance Report — %s", ist.strftime("%d %b %Y %I:%M %p IST"))

    job_results   = []
    audit_results = []

    for env_key in ENVIRONMENTS:
        logging.info("[%s] jobs ...", env_key)
        jm = get_job_metrics(env_key, now)
        job_results.append(jm)
        if jm["error"]:
            logging.warning("[%s] jobs ERROR: %s", env_key, jm["error"][:70])
        else:
            logging.info("[%s] jobs total=%d pending=%d compliance=%.1f%%",
                         env_key, jm["total_accts"], jm["n2_pending"], jm["compliance_pct"])

        logging.info("[%s] audit ...", env_key)
        am = get_audit_metrics(env_key, now)
        audit_results.append(am)
        if am["error"]:
            logging.warning("[%s] audit ERROR: %s", env_key, am["error"][:70])
        else:
            logging.info("[%s] audit requests=%d slow=%d p95=%s p99=%s",
                         env_key, am["total_requests"], am["slow_count"],
                         "Good" if am["p95_good"] else "Bad",
                         "Good" if am["p99_good"] else "Bad")

    logging.info("Building HTML report...")
    html_str = build_html(now, job_results, audit_results)
    html_bytes = html_str.encode("utf-8")

    logging.info("Building CSV raw data dump...")
    csv_bytes = generate_csv_bytes(now)

    html_filename = f"corestack-performance-report.html"
    csv_filename  = f"corestack_section1_raw_{ist.strftime('%d%b%Y_%I%M%p')}.csv"

    logging.info("Report complete: HTML=%d bytes, CSV=%d bytes",
                 len(html_bytes), len(csv_bytes))

    return html_bytes, csv_bytes, html_filename, csv_filename
